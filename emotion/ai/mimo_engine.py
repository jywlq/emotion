...
class MiMoEngine:
    ...
    def api_key(self, value: str):
        ...
        if value:
            _save_config({"api_key": value, "model": self._model, "base_url": self._base_url})
    ...
    def model(self, value: str):
        ...
        self._client = None
        if self._api_key:
            _save_config({"api_key": self._api_key, "model": value, "base_url": self._base_url})
    ...
    def _chat(self, system_prompt: str, user_message: str, max_tokens: int = 512) -> str:
        ...
        try:
            ...
            log_api_call(
                engine="MiMo",
                function=system_prompt[:30] + "...",
                prompt_tokens=prompt_tokens,
                completion_tokens=completion_tokens,
                latency_ms=latency,
                success=True,
                metadata={"model": self._model, "max_tokens": max_tokens},
            )
            if content:
